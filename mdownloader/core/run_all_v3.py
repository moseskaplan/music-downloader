"""Orchestrate the music downloader pipeline with separate selection and download steps.

This script coordinates the parsing, selection, downloading and tagging steps
for one or more provided media URLs.  It handles automatic parser
selection based on the URL domain, supports test mode output to a
temporary directory, logs each operation, and writes a run log into
each album folder for easy troubleshooting.

Compared to the original ``run_all.py``, this version introduces a
dedicated selection step.  After parsing an album into a CSV, it
invokes ``mdownloader.services.track_selector`` to populate
``selected_url`` and ``selection_flag`` columns.  Then it calls
``track_download_v3.py`` (or the simplified ``track_download``
module) to download the selected URLs, followed by tagging via
``track_metadata_cleanup``.

Usage::

    python3 run_all_v3.py --url <url1> [<url2> ...]

Use ``--type`` to force a specific parser (wiki, youtube, search,
apple).  When omitted, the parser type is inferred from each URL.
Additional flags include ``--workers`` to set concurrency for the
download step, ``--skip-parse``, ``--skip-select``, ``--skip-download``,
``--skip-tag`` and ``--test-mode``.
"""
...
# === CLI args ===
parser = argparse.ArgumentParser(description="Cascade music downloader scripts with selection.")
...
parser.add_argument("--skip-select", action="store_true", help="Skip selection step (use existing selected_url values)")
...
# === API key check ===
# Ensure an API key is available for the selection step
if not args.skip_select:
    api_key = os.getenv("YOUTUBE_API_KEY")
    if not api_key:
        write_log("[🛑 ERROR] YOUTUBE_API_KEY environment variable not set. "
                  "Please export it in your shell config or set it before running.")
        sys.exit(1)
    ...
    for url in args.url:
        ...
        if not args.skip_parse:
            ...
            if not csv_path:
                ...
                continue
        # === STEP 2: Select ===
        if not args.skip_select and csv_path:
            # Pass API key implicitly via environment; no extra args needed
            if not run_step(2, "Selecting", "mdownloader.services.track_selector", [csv_path]):
                continue
        # === STEP 3: Download ===
        if not args.skip_download and csv_path:
            dl_args = [csv_path]
            if args.workers:
                dl_args += ["--workers", str(args.workers)]
            if not run_step(3, "Downloading", "mdownloader.services.track_download", dl_args):
                continue
        # === STEP 4: Tag ===
        if not args.skip_tag and csv_path:
            if not run_step(4, "Tagging", "mdownloader.services.track_metadata_cleanup", [csv_path]):
                continue
