...
def tag_album_tracks(csv_path, test_mode=False):
    ...
    for _, row in df.iterrows():
        ...
        if pd.isna(track_number) or track_number == "" or track_number is None:
            key = 1  # fallback for single tracks
        else:
            try:
    ...
    for fname in os.listdir(album_dir):
        ...
        if test_mode:
            print(f"[TEST-MODE] Would tag: {fname} → "
                  f"{row['track_title']} / {row['artist_name']} / {row['album_name']}")
            continue
